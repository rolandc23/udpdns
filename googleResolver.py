...
def makeQuery(domainName, queryType):
    ...
    header = Header(id = randint(0, MAX_H), qdCount = 1, flags = 1 << 8, anCount = 0, nsCount = 0, arCount = 0)
...
def sendQuery(ns, domainName, type, timeout, last):
    ...
    try:
        ...
        if last:
            raise TimeoutError('ERROR: Timed out - Server failure or the address you have queried could not be resolved')
